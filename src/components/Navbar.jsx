...
function Navbar() {
  ...
  const authLinks = (<><Link className="flex items-center hover:text-gray-200" to='/logout'>
      ...
      <div className="flex flex-wrap h-screen h-28">
        <section className="relative mx-auto ">
         ...
         flex-col bg-black drop-shadow-lg overflow-visible absolute top-10">
                    {data?.map(category => {
                      return (
                        <Link key={category.id} className="px-5 py-3 hover:bg-gray-900 ">{category.name}</Link>
                      )
                    })}
              ...
              <div className="hidden xl:flex items-center space-x-5 items-center">
                {/* favorites */}
                <button className="hover:text-gray-200" href="#">
                ...
                <button className="flex items-center hover:text-gray-200" data-drawer-target="cart-container">
                  <svg onClick={() => alert("jjjj")} xmlns="http://www.w3.org/2000/svg" className=" h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
